...
class User:
    ...
    def login(self, req_post):
        ...
        if (username != "" and password != ""):
            ...
            while row is not None:
                # print(row["COUNT(1)"])
                if (row["COUNT(1)"] == 1):
                    cur.execute("SELECT user_pwd,user_id,fname,lname FROM user WHERE user_name = %s;",
                                username)  # FETCH THE HASHED PASSWORD
    ...
    def getuser(self, userid):
        uid = userid
        cur.execute(
            "SELECT fname, lname, email, company_id, channel_ids, user_type_id, status, fb_handle, twt_handle, insta_handle, profile_pic, bio FROM user WHERE user_id = %s;",
            [uid])
